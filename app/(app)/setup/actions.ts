...
export type InviteOutcome = {
  name: string;
  email: string;
  /** Shown once, here, so the owner can hand it over. Never stored in clear. */
  password: string;
};
 ...
 *
 * That action requires a password, because RepairFlow has no invite-email flow
 * yet. Rather than ask an owner to invent three of them on their first day,
 * a strong one is generated per person and shown once so it can be handed over
 * in the room. The generated value is never stored in the clear: it goes
 * straight into bcrypt inside the action, and this return value is the only
...
> {
  ...
  for (const row of wanted) {
    ...
    invited.push({ name: row.name, email: row.email, password });
  }
...
/**
 * 16 URL-safe characters from the CSPRNG — comfortably past the 8-character
 * floor the invite action enforces, and short enough to read aloud once.
 */
