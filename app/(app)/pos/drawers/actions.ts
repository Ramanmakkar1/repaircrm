 ...
 *
 * There is no `lib/location.ts` in the app yet, so a session opens the shop's
 * drawer with `locationId: null`. The column exists and the queries are already
 * written per-shop, so adding the location filter later is one `where` clause.
 */
