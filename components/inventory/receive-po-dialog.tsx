            ...
            {outstanding.map((line) => {
              ...
              return (
                <div key={line.id} className="flex flex-col gap-3 px-4 py-3.5">
                  ...
                  {/* One scan control per line sits on the label row,
                      appending a line to the box per camera read — receiving a
                      box of serialized units is the heaviest typing job in the
                      app and the one a phone camera actually fixes. */}
                  {line.serialized ? (
                    <SerialScanField
                      id={`serials-${line.id}`}
