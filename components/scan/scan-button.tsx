...
export function ScanButton({
  ...
  showLabel?: boolean;
  /**
   * Classes for the visible word, so a caller can drop it at a narrow width
   * without losing the accessible name — the POS slot is icon-plus-word on a
   * laptop and a 56px square at 390px, where the word would eat the search box.
   */
  ...
  return (
    <>
      <Button
        ...
        disabled={disabled}
        // The word can be hidden responsively, so the name is always spelled out.
        aria-label={label}
        title={label}
      >
        <ACTIONS.scan />
