    ...
    <Card>
      <CardHeader className="flex-row items-center gap-3.5">
        ...
        <div className="flex min-w-0 flex-col gap-1">
          ...
          <CardDescription>
            One link for everything. Put it on your website, your Google
            listing, your receipts and the sticker in the window.
          </CardDescription>
        </div>
      </CardHeader>
...
function ConnectionsCard({ config }: { config: ConnectConfig }) {
  ...
  const rows: ConnectRowProps[] = [
    ...
    {
      // The camera-scanning screens land on their own branch; until they do,
      // this row is an honest explanation and a way into the register rather
      // than a promise. Swap the body line when components/scan/ arrives.
      icon: ScanLine,
      title: "Scan with your phone",
      body: "You don't need a scanner gun. The register takes a barcode from any device you already own, or a code typed by hand.",
      status: "connected",
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>Connect everything else</CardTitle>
        <CardDescription>
          Top to bottom, one button each. Nothing here is set up twice — every
          row opens the screen that actually owns it.
        </CardDescription>
      </CardHeader>
...
function StatusPill({
  ...
  return (
    <span
      className={cn(
        "inline-flex w-fit items-center gap-1.5 rounded-full px-2.5 py-1 text-[12px] font-semibold leading-none",
        status === "connected"
          ? "bg-status-resolved-bg text-status-resolved-fg"
          : status === "attention"
            ? "bg-status-in-progress-bg text-status-in-progress-fg"
